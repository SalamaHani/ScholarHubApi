...
describe("Data Validation Tests", () => {
  describe("Input Validation", () => {
    test("Should validate user registration input", () => {
      ...
      const invalid1 = validateRegistration({
        email: "invalid",
        password: "short",
        name: "",
      });
    ...
    test("Should validate scholarship application input", () => {
      ...
      expect(() =>
        validateScholarshipApp({
          scholarshipId: 1,
          applicantId: 1,
          essayText: "short",
        }),
      ).toThrow("Essay must be at least 100 characters");
    ...
    test("Should validate document upload input", () => {
      ...
      expect(() =>
        validateDocumentUpload({ fileName: "doc.exe", fileSize: 1000 }),
      ).toThrow("File format must be one of");
      expect(() =>
        validateDocumentUpload({
          fileName: "doc.pdf",
          fileSize: 10 * 1024 * 1024,
        }),
      ).toThrow("File size must not exceed 5MB");
  ...
  describe("Business Logic Validation", () => {
    test("Should validate scholarship requirements", () => {
      ...
      expect(() => validateScholarship({})).toThrow(
        "Scholarship title is required",
      );
      expect(() =>
        validateScholarship({ title: "Test", amount: -100 }),
      ).toThrow("Scholarship amount must be greater than 0");
      expect(() =>
        validateScholarship({ title: "Test", amount: 1000, maxGPA: 5.0 }),
      ...
      ).toThrow("Minimum GPA cannot be greater than maximum GPA");
    ...
    test("Should validate interview scheduling", () => {
      const validateInterviewSchedule = (data: any) => {
        ...
        if (daysDiff < 1) {
          throw new Error(
            "Interview must be scheduled at least 1 day in advance",
          );
        }
  ...
  describe("Range Validation", () => {
    ...
    test("Should validate array length", () => {
      ...
      expect(() => validateTags(["a", "b", "c", "d", "e", "f"])).toThrow(
